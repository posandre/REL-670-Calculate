...
def configure_rx_axes(axis: Axes, labels: dict[str, str]) -> None:
    ...
    axis.xaxis.set_minor_locator(MultipleLocator(50 / 3))
    axis.yaxis.set_minor_locator(MultipleLocator(50 / 3))
...
def plot_rx_diagram(
    ...
    for zone in zones:
        ...
        axis.plot(xs, ys, linewidth=1.0, label=zone.name)
    ...
    if psb is not None:
        ...
        axis.plot(outer_x, outer_y, color="#8b5cf6", linewidth=1.0, label=labels["psb_outer"])
        axis.plot(inner_x, inner_y, color="#ef4444", linewidth=1.0, label=labels["psb_inner"])
