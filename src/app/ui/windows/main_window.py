...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self._distance_phase_phase_zone_fills: dict[str, object] = {}
        self._distance_phase_ground_zone_fills: dict[str, object] = {}
    ...
    def _distance_zone_fill_artist(self, graph_kind: str, zone_name: str) -> object | None:
        if graph_kind == "phase_phase":
            return self._distance_phase_phase_zone_fills.get(zone_name)
        return self._distance_phase_ground_zone_fills.get(zone_name)
    ...
    def _connect_distance_drag_handlers(self, graph_kind: str) -> None:
        ...
        def press(event) -> None:  # type: ignore[no-untyped-def]
            ...
            fill = self._distance_zone_fill_artist(target.graph_kind, target.zone_name)
            if fill is not None:
                cast(Any, fill).set_visible(False)
        ...
        def release(event) -> None:  # type: ignore[no-untyped-def]
            if self._distance_drag_active is None:
                return
            graph_kind_released = self._distance_drag_active.graph_kind
            zone_name_released = self._distance_drag_active.zone_name
            self._distance_drag_active = None
            self._distance_drag_limits = None
            fill = self._distance_zone_fill_artist(graph_kind_released, zone_name_released)
            if fill is not None and self._show_zone_fill:
                cast(Any, fill).set_visible(True)
            canvas.setCursor(self._toolbar_mode_cursor(panel))
            self._redraw_distance_graph(graph_kind_released)
    ...
    def _plot_distance_phase_phase_zones(self) -> None:
        ...
        self._distance_phase_phase_point_targets = []
        self._distance_phase_phase_drag_targets = []
        self._distance_phase_phase_zone_fills = {}
        active_zone = (
            self._distance_drag_active.zone_name
            if self._distance_drag_active is not None
            and self._distance_drag_active.graph_kind == "phase_phase"
            else None
        )
        for raw_stage, zone in zip(raw_stages, zones, strict=False):
            ...
            if self._show_zone_fill:
                ...
                fill.set_visible(visible and zone.name != active_zone)
                self._distance_phase_phase_zone_fills[zone.name] = fill
            line_by_label[zone.name] = line
            if visible:
    ...
    def _plot_distance_phase_ground_zones(self) -> None:
        ...
        self._distance_phase_ground_zone_fills = {}
        active_zone = (
            self._distance_drag_active.zone_name
            if self._distance_drag_active is not None
            and self._distance_drag_active.graph_kind == "phase_ground"
            else None
        )
        for raw_stage, zone in zip(raw_stages, zones, strict=False):
            xs = [point[0] for point in zone.points]
            ...
            if self._show_zone_fill:
                ...
                fill.set_visible(visible and zone.name != active_zone)
                self._distance_phase_ground_zone_fills[zone.name] = fill
            line_by_label[zone.name] = line
            if visible:
                self._register_distance_drag_target(
                    "phase_ground",
    ...
    def _open_settings(self) -> None:
        dialog = SettingsDialog(
            ...
            show_zone_fill=self._show_zone_fill,
        ...
        if dialog.exec():
            ...
            try:
                ...
                self._show_zone_fill = dialog.show_zone_fill
